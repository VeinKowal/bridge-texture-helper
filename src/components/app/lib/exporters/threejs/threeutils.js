  ...
  for (let originalMaterial of originalMaterials) {
    if (typeToHighlightMaterial.has(originalMaterial.type)) {
      highlightMaterials.push(
        typeToHighlightMaterial.get(originalMaterial.type)
      );
      continue;
    }
    let highlightMaterial = CreateHighlightMaterial(
      originalMaterial,
      highlightColor,
      withPolygonOffset
    );
...
export function GetLineSegmentsProjectedDistance(
  camera,
  canvasWidth,
  canvasHeight,
  lineSegments,
