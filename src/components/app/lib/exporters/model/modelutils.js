...
export function IsTwoManifold(object3D) {
  ...
  if (object3D instanceof Model) {
    ...
    for (let edgeIndex = 0; edgeIndex < topology.edges.length; edgeIndex++) {
      ...
      let edgeOrientation1 = GetEdgeOrientationInTriangle(
        topology,
        edge.triangles[0],
        edgeIndex
      );
      let edgeOrientation2 = GetEdgeOrientationInTriangle(
        topology,
        edge.triangles[1],
        edgeIndex
      );
      if (
        edgeOrientation1 === null ||
        edgeOrientation2 === null ||
        edgeOrientation1 === edgeOrientation2
      ) {
        return false;
      }
    }
