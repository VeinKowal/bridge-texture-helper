...
class ModelFinalizer {
  ...
  FinalizeMesh(model, mesh) {
    ...
    for (let i = 0; i < mesh.LineCount(); i++) {
      ...
      if (line.mat === null) {
        line.mat = this.GetDefaultMaterialIndex(
          model,
          MaterialSource.DefaultLine
        );
      }
    }
    ...
    for (let i = 0; i < mesh.TriangleCount(); i++) {
      ...
      if (triangle.mat === null) {
        triangle.mat = this.GetDefaultMaterialIndex(
          model,
          MaterialSource.DefaultFace
        );
      }
    }
  ...
  GetDefaultMaterialIndex(model, source) {
    ...
    if (source === MaterialSource.DefaultLine) {
      this.defaultLineMaterialIndex = GetIndex(
        model,
        this.defaultLineMaterialIndex,
        MaterialSource.DefaultLine,
        this.params.defaultLineMaterialColor
      );
      ...
      this.defaultMaterialIndex = GetIndex(
        ...
        MaterialSource.DefaultFace,
        this.params.defaultMaterialColor
      );
