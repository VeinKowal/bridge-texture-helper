...
export class ImporterObj extends ImporterBase {
  ...
  ProcessLineCommand(parameters) {
    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      let vertexIndex = this.GetRelativeIndex(
        parseInt(vertexParams[0], 10),
        this.globalVertices.length
      );
      let meshVertexIndex = this.currentMeshConverter.AddVertex(
        vertexIndex,
        this.globalVertices
      );
  ...
  ProcessFaceCommand(parameters) {
    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      if (vertexParams.length > 1 && vertexParams[1].length > 0) {
        uvs.push(
          this.GetRelativeIndex(
            parseInt(vertexParams[1], 10),
            this.globalUvs.length
          )
        );
      }
